...
function calcZoom(viewRect, goToRect, margin) {
    if (goToRect.aspectRatio > viewRect.aspectRatio) {
        return parseFloat((viewRect.actualWidth - (2 * margin)) / goToRect.width);
    } else {
        return parseFloat((viewRect.actualHeight - (2 * margin)) / goToRect.height);
    }
}
...
function fitSel(document) {
    ...
    for (i = 1; i <= amountofsteps; i++) {
        ...
        if (zoomDelta > 0) { // ZOOM IN
            zoomie = easeInCubic(i, startzoom, zoomDelta, amountofsteps); //easeInCubic
            centerX = easeInCubic(i, startpoint[0], Xdelta, amountofsteps);
            centerY = easeInCubic(i, startpoint[1], Ydelta, amountofsteps);
        } else { // ZOOM OUT
            zoomie = easeOutCubic(i, startzoom, zoomDelta, amountofsteps); //easeOutQuart
            centerX = easeOutCubic(i, startpoint[0], Xdelta, amountofsteps);
            centerY = easeOutCubic(i, startpoint[1], Ydelta, amountofsteps);
        }
