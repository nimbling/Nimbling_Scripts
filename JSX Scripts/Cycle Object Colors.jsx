...
//  script.name = Cycle Object Colors.jsx;
    ...
    if (sel && sel.length > 0) {
        if (isContainerLockedOrInvisible(curDoc.activeLayer)) {
            ...
            if (flatSelection.length >= 2) {
                CycleObjAppearance(flatSelection);
            } else {
...
function CycleObjAppearance(sel) {
    var appearances = [];
    ...
    for (var j = 0; j < sel.length; j++) {
        var targetIndex = (j + 1) % sel.length;
        PasteAppearance(sel[j], appearances[targetIndex]);
    }
}
